...
export function PricingTable({
  ...
  // Check if any plan has a quarterly price option
  const hasQuarterlyOption = subscriptionPlans.some((plan) =>
    plan.prices.some(
      (price) =>
        price.type === PaymentTypes.SUBSCRIPTION &&
        price.interval === PlanIntervals.QUARTER
    )
  );
  ...
  return (
    <div className={cn('flex flex-col gap-12', className)}>
      ...
      {(hasMonthlyOption || hasYearlyOption || hasQuarterlyOption) &&
